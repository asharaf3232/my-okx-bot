// =================================================================
// Advanced Analytics Bot - v400 (Stable Polling & Corrected DB Logic)
// =================================================================
...
// =================================================================
// SECTION 2: BOT LOGIC AND HANDLERS (Telegraf Polling Version)
// =================================================================
bot.use((ctx, next) => { if (ctx.from?.id === AUTHORIZED_USER_ID) return next(); console.log(`Unauthorized access: ${ctx.from?.id}`); });
const mainKeyboard = Markup.keyboard([['📊 عرض المحفظة', '📈 أداء المحفظة']]).resize();
...
bot.start((ctx) => ctx.replyWithMarkdown('🤖 *أهلاً بك في بوت التحليل المتكامل.*', mainKeyboard));
