...
class OKXAdapter {
    ...
    async getMarketPrices() {
        try {
            ...
            if (tickersJson.code !== '0') {
                ...
                return { error: `فشل جلب أسعار السوق من OKX: ${tickersJson.msg}` };
            }
            const prices = {};
            ...
            return { error: "خطأ استثنائي عند جلب أسعار OKX." };
        }
    }
    ...
    async getBalanceForComparison() { /* ... This function is OKX-specific ... */ return null; }
}

// --- KuCoin Adapter (Fully Implemented) ---
class KuCoinAdapter {
    constructor() {
        this.name = "KuCoin";
    ...
    async getPortfolio(prices) {
        try {
            ...
            json.data.forEach(account => {
                ...
                if (amount > 0) {
                    if (combinedBalances[account.currency]) combinedBalances[account.currency] += amount;
                    else combinedBalances[account.currency] = amount;
                }
            });
...
class GateIoAdapter {
    ...
    async getMarketPrices() {
        try {
            ...
            if (!Array.isArray(json)) {
                 return { error: `فشل جلب أسعار السوق من Gate.io: ${json.message || 'بيانات غير متوقعة'}` };
            }
            const prices = {};
            json.forEach(t => {
                if (t.currency_pair.endsWith('_USDT')) {
                    const instId = t.currency_pair.replace('_', '-');
                    prices[instId] = {
                        price: parseFloat(t.last),
                        change24h: parseFloat(t.change_percentage) / 100,
                        volCcy24h: parseFloat(t.quote_volume)
                    };
                }
            });
            return prices;
        } catch (error) {
            console.error("Exception in GateIoAdapter.getMarketPrices:", error.message);
            return { error: "خطأ استثنائي عند جلب أسعار Gate.io." };
        }
    }
    ...
    async getPortfolio(prices) {
        try {
            ...
            if (!Array.isArray(json)) {
                console.error("Gate.io Portfolio Error:", json);
                return { error: `فشل جلب المحفظة من Gate.io: ${json.message || 'بيانات غير متوقعة'}` };
            }

            let assets = [], total = 0, usdtValue = 0;
            for (const account of json) {
                const amount = parseFloat(account.available) + parseFloat(account.locked); // Combine available and locked
                if (amount > 0) {
                    ...
                    if (ccy === "USDT") usdtValue = value;
                    if (value >= 1) {
                        assets.push({ asset: ccy, price: priceData.price, value, amount, change24h: priceData.change24h });
                    }
                }
            }
            ...
            console.error("Exception in GateIoAdapter.getPortfolio:", e);
            return { error: "خطأ استثنائي في الاتصال بمنصة Gate.io." };
        }
    }
...
async function runDailyJobs() { try { const adapter = getAdapter(); if (adapter.name !== 'OKX') return; const settings = await loadSettings(); if (!settings.dailySummary) return; const prices = await adapter.getMarketPrices(); if (!prices || prices.error) return; const { total } = await adapter.getPortfolio(prices); if (total === undefined) return; const history = await loadHistory(); const date = new Date().toISOString().slice(0, 10); const todayIndex = history.findIndex(h => h.date === date); if (todayIndex > -1) history[todayIndex].total = total; else history.push({ date, total }); if (history.length > 35) history.shift(); await saveHistory(history); console.log(`[Daily Summary Recorded]: ${date} - $${formatNumber(total)}`); } catch (e) { console.error("CRITICAL ERROR in runDailyJobs:", e); } }
async function runHourlyJobs() { try { const adapter = getAdapter(); if (adapter.name !== 'OKX') return; const prices = await adapter.getMarketPrices(); if (!prices || prices.error) return; const { total } = await adapter.getPortfolio(prices); if (total === undefined) return; const history = await loadHourlyHistory(); const hourLabel = new Date().toISOString().slice(0, 13); const existingIndex = history.findIndex(h => h.label === hourLabel); if (existingIndex > -1) history[existingIndex].total = total; else history.push({ label: hourLabel, total }); if (history.length > 72) history.splice(0, history.length - 72); await saveHourlyHistory(history); } catch (e) { console.error("Error in hourly jobs:", e); } }
async function monitorVirtualTrades() { const adapter = getAdapter(); if (adapter.name !== 'OKX') return; const activeTrades = await getActiveVirtualTrades(); if (activeTrades.length === 0) return; const prices = await adapter.getMarketPrices(); if (!prices || prices.error) return; for (const trade of activeTrades) { const currentPrice = prices[trade.instId]?.price; if (!currentPrice) continue; let finalStatus = null; let pnl = 0; let finalPrice = 0; if (currentPrice >= trade.targetPrice) { finalPrice = trade.targetPrice; pnl = (finalPrice - trade.entryPrice) * (trade.virtualAmount / trade.entryPrice); finalStatus = 'completed'; const profitPercent = (trade.virtualAmount > 0) ? (pnl / trade.virtualAmount) * 100 : 0; const msg = `🎯 *الهدف تحقق (توصية افتراضية)!* ✅\n\n` + `*العملة:* \`${trade.instId}\`\n` + `*سعر الدخول:* \`$${formatNumber(trade.entryPrice, 4)}\`\n` + `*سعر الهدف:* \`$${formatNumber(trade.targetPrice, 4)}\`\n\n` + `💰 *الربح المحقق:* \`+$${formatNumber(pnl)}\` (\`+${formatNumber(profitPercent)}%\`)`; await bot.api.sendMessage(AUTHORIZED_USER_ID, msg, { parse_mode: "Markdown" }); } else if (currentPrice <= trade.stopLossPrice) { finalPrice = trade.stopLossPrice; pnl = (finalPrice - trade.entryPrice) * (trade.virtualAmount / trade.entryPrice); finalStatus = 'stopped'; const lossPercent = (trade.virtualAmount > 0) ? (pnl / trade.virtualAmount) * 100 : 0; const msg = `🛑 *تم تفعيل وقف الخسارة (توصية افتراضية)!* 🔻\n\n` + `*العملة:* \`${trade.instId}\`\n` + `*سعر الدخول:* \`$${formatNumber(trade.entryPrice, 4)}\`\n` + `*سعر الوقف:* \`$${formatNumber(trade.stopLossPrice, 4)}\`\n\n` + `💸 *الخسارة:* \`$${formatNumber(pnl)}\` (\`${formatNumber(lossPercent)}%\`)`; await bot.api.sendMessage(AUTHORIZED_USER_ID, msg, { parse_mode: "Markdown" }); } if (finalStatus) { await updateVirtualTradeStatus(trade._id, finalStatus, finalPrice); } } }

function stopOkxJobs() { if (backgroundJobIds.length > 0) { console.log("Stopping OKX-specific background jobs..."); backgroundJobIds.forEach(id => clearInterval(id)); backgroundJobIds = []; bot.api.sendMessage(AUTHORIZED_USER_ID, "ℹ️ تم إيقاف المراقبة المتقدمة مؤقتاً.").catch(console.error); } }
function startOkxJobs() { stopOkxJobs(); console.log("Starting OKX-specific background jobs..."); backgroundJobIds.push(setInterval(monitorBalanceChanges, 60 * 1000)); backgroundJobIds.push(setInterval(trackPositionHighLow, 60 * 1000)); backgroundJobIds.push(setInterval(checkPriceAlerts, 30 * 1000)); backgroundJobIds.push(setInterval(checkPriceMovements, 60 * 1000)); backgroundJobIds.push(setInterval(monitorVirtualTrades, 30 * 1000)); backgroundJobIds.push(setInterval(runHourlyJobs, 60 * 60 * 1000)); backgroundJobIds.push(setInterval(runDailyJobs, 24 * 60 * 60 * 1000)); bot.api.sendMessage(AUTHORIZED_USER_ID, "✅ تم تفعيل المراقبة المتقدمة لمنصة OKX.").catch(console.error); }

// =================================================================
// SECTION 5: BOT SETUP, KEYBOARDS, AND HANDLERS
// =================================================================
const buildMainKeyboard = () => new Keyboard()
...
bot.command("start", (ctx) => { const welcomeMessage = `🤖 *أهلاً بك في بوت التحليل المتكامل، مساعدك الذكي لإدارة وتحليل محفظتك الاستثمارية.*\n\n` + `*المنصة النشطة حالياً:* ${getAdapter().name}\n\n` + `*اضغط على الأزرار أدناه للبدء!*`; ctx.reply(welcomeMessage, { parse_mode: "Markdown", reply_markup: buildMainKeyboard() }); });
