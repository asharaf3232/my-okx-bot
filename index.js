// =================================================================
// OKX Advanced Analytics Bot - v114 (Exchange Switching Logic Fixed)
// =================================================================
...
class OKXAdapter {
    ...
    async getBalanceForComparison() {
        try {
            ...
            console.error("Error in getBalanceForComparison:", e);
            return null;
        }
    }
}
    ...
    async getPortfolio(prices) {
        try {
            ...
            for (const account of json) {
                const amount = parseFloat(account.available) + parseFloat(account.locked);
                if (amount > 0) {
...
bot.on("callback_query:data", async (ctx) => {
    ...
    try {
        // --- NEW, ROBUST EXCHANGE SWITCHING LOGIC ---
        if (data.startsWith("set_exchange_")) {
            const newEx = data.split('_')[2];
            if (adapters[newEx]) {
                // Always stop any potentially running jobs first, regardless of the current state.
                stopOkxJobs();

                // Set the new active exchange
                activeExchange = newEx;
                await ctx.editMessageText(`✅ تم التبديل إلى منصة ${getAdapter().name}.`);
                await ctx.reply("تم تحديث لوحة التحكم.", { reply_markup: buildMainKeyboard() });

                // If the newly selected exchange is OKX, explicitly start the jobs.
                if (activeExchange === 'okx') {
                    startOkxJobs();
                }
            } else {
                 await ctx.editMessageText("منصة غير معروفة.");
            }
            return;
        }
        // --- END OF NEW LOGIC ---
        ...
        switch(data) {
            ...
            case "track_virtual_trades":
                ...
                const prices = await adapter.getMarketPrices();
                if (!prices || prices.error) {
                    await ctx.editMessageText(`❌ فشل جلب الأسعار من ${adapter.name}، لا يمكن متابعة التوصيات.`, { reply_markup: virtualTradeKeyboard });
                    return;
                }
...
async function startBot() {
    try {
        ...
        // Check the default exchange on startup
        if (activeExchange === 'okx') {
            startOkxJobs();
