// =================================================================
// Advanced Analytics Bot - v134.5 (Calculator Added Directly to Stable v134.1)
// =================================================================
...
// --- State Variables ---
// تم الترقية من waitingState لدعم محادثات متعددة ومتزامنة
const userStates = {};
...
async function getHistoricalCandles(instId, bar = '1D', limit = 100) {
    ...
    try {
        while (allCandles.length < limit) {
            ...
            const newCandles = json.data.map(c => ({ time: parseInt(c[0]), high: parseFloat(c[2]), low: parseFloat(c[3]), close: parseFloat(c[4]) }));
...
bot.on("message:text", async (ctx) => {
    ...
    // إذا لم يكن المستخدم في محادثة، لا تفعل شيئًا.
    // الأوامر الأخرى مثل الأزرار ستتم معالجتها في مكان آخر.
    if (!state) return;
            ...
            case 'targets':
                if (text.toLowerCase() === 'تم' || text.toLowerCase() === 'done') {
                    ...
                    if (totalPercentage !== 100) {
                        return await ctx.reply(`إجمالي النسب المئوية لأهدافك هو ${totalPercentage}%. يجب أن يكون المجموع 100%.`);
                    }
                ...
                let replyMsg = `تم إضافة الهدف: بيع ${percentage}% عند سعر $${price}.\n`;
...
// =================================================================
// SECTION 6: BOT INITIALIZATION & BACKGROUND TASKS
// =================================================================
(async () => {
    try {
        await connectDB();
        ...
        bot.start({
            ...
            allowed_updates: ["message"], // يستمع للرسائل فقط
            onStart: (botInfo) => {
